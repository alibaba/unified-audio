...
from .vq import Codec_stereo


class Model_stereo(pl.LightningModule):
    def __init__(self, config):
        super().__init__()
        self.config = config

        self.generator = Codec_stereo(config['encoder_config'], config['decoder_config'], config['quantizer_config'], config['adaptive_config'])
    ...
    def forward(self, wav):
        # wav: (b, 2, t)
        feat = self.extract_wav2vec2_features((wav[:,0]+wav[:,1])*0.5).transpose(-2, -1).detach()
        recon, pred_feat, commit_loss = self.generator(wav, feat)
        return recon

    def test_step(self, batch, batch_idx):
        wav, spatial_info, fs, lengths, names = batch
        # stereo input: wav is (b, 2, t)
        recon = self(self.pad_wav(wav))
